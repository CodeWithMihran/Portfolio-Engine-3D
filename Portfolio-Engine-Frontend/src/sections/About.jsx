      ...
      <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/8 to-white/5 p-8 backdrop-blur">
          ...
          <p className="mt-6 text-base leading-8 text-white/70">
            {profile?.about ||
              "This portfolio combines product thinking, backend structure, and interactive visuals so every section feels intentional and alive."}
          </p>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {socialItems.map((item) => (
                <a
                  key={item.key}
                  href={links[item.key] || "#"}
                  target={links[item.key] ? "_blank" : undefined}
                  rel={links[item.key] ? "noreferrer" : undefined}
                ...
                >
                  ...
                  <p className="mt-1 truncate font-medium text-white/85">
                    {links[item.key] || "Add this from admin later"}
                  </p>
                </a>
              ))}
            </div>
