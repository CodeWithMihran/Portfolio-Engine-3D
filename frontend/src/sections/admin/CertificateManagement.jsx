...
export default function CertificateManagement() {
  ...
  // DEBUG: Monitor state changes in the console as you type
  useEffect(() => {
  ...
  const handleSubmit = async (event) => {
    ...
    // 🔍 DEBUG: Check what is actually inside 'form' right now
    console.log("🚀 SUBMIT CLICKED! Final Payload Check:", form);

    if (!form.title || !form.issuer || !form.issueDate) {
      console.error("❌ STOPPED: Required fields are missing in state!");
      alert("Please fill in Title, Issuer, and Issue Date.");
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(7,14,28,0.92),rgba(9,17,31,0.78))] p-6 shadow-[0_24px_70px_rgba(2,6,23,0.3)] backdrop-blur-2xl">
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="grid gap-4 lg:grid-cols-2">
            {/* Added 'required' attributes to trigger browser warnings if state is empty */}
            <input required className="rounded-[22px] border border-white/10 bg-slate-950/85 p-4 outline-none" placeholder="Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            <input required className="rounded-[22px] border border-white/10 bg-slate-950/85 p-4 outline-none" placeholder="Issuer" value={form.issuer} onChange={(e) => setForm({ ...form, issuer: e.target.value })} />
            <input required type="date" className="rounded-[22px] border border-white/10 bg-slate-950/85 p-4 outline-none" value={form.issueDate} onChange={(e) => setForm({ ...form, issueDate: e.target.value })} />
        ...
        <div className="flex gap-3">
          <button type="submit" className="rounded-[22px] bg-[linear-gradient(135deg,#06b6d4,#2563eb)] px-5 py-4 text-sm font-semibold text-white transition hover:brightness-110">
